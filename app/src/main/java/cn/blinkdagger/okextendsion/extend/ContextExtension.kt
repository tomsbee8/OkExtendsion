...
inline fun ComponentActivity.launchActivityForResult(
    intent: Intent,
...
inline fun <reified T : Activity> ComponentActivity.launchActivityForResult(crossinline onActivityResult: (ActivityResult) -> Unit) {
    val intent = AnkoInternals.createIntent(this, T::class.java, emptyArray())
    this.launchActivityForResult(intent, onActivityResult)
}

inline fun <reified T : Activity> ComponentActivity.launchActivityForResult(
...
inline fun ComponentActivity.requestPermission(
    permission: String,
    crossinline onPermissionResult: (Boolean) -> Unit
) {
...
inline fun ComponentActivity.requestPermission(
    ...
    crossinline onPermissionResult: (Map<String, Boolean>) -> Unit
) {
...
fun Context?.aliveActivity(): Context? {
...
fun <T : Parcelable> Activity?.arrayListExtra(
    key: String,
...
fun <T : Parcelable> Fragment?.arrayListExtra(
    key: String,
    defaultValue: ArrayList<T> = arrayListOf()
): ArrayList<T> {
...
fun Context?.dp2px(dp: Float): Int = this?.aliveActivity()?.let { return Math.round(dp * (it.resources.displayMetrics.xdpi / DisplayMetrics.DENSITY_DEFAULT)) }?: 0
fun Context?.sp2px(sp: Int): Int = sp2px(sp.toFloat())
fun Context?.sp2px(sp: Float): Int = this?.aliveActivity()?.let { return (sp * it.resources.displayMetrics.scaledDensity + 0.5f).toInt() }?: 0
